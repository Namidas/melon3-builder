...
function smarty_block_docs_class_definition($params,$content,$template,&$repeat)
{	
	...
	if(!$repeat)
	{
		...
		if($params['render_template'] === true) return builder_render_template_code('docs_class_definition',$params,$template,$content);
		else
		{
	...
	else smarty_append_nested($template,'docs_class_definition',['type' => $params['type']],smarty_get_map_data_from_params($params));
}

function smarty_block_docs_class_member($params,$content,$template,&$repeat)
{
	...
	if(!$repeat)
	{
		...
		if($params['render_template'] === true) return builder_render_template_code('docs_class_member',$params,$template,$content);
		else
		{
			$content_parsed = smarty_content_formater($content,$params);
			$cssclass = smarty_class_param($params);
	...
	else smarty_append_nested($template,'docs_class_member',[],smarty_get_map_data_from_params($params));
}
